...
public interface MatchRepository extends JpaRepository<Match, UUID> {
    ...
    long countVictories(User player);

    @Query("SELECT COUNT(m) FROM Match m WHERE m.status = 'FINISHED' AND (m.player1 = :player OR m.player2 = :player) AND m.winner != :player")
    long countDefeats(User player);
}
